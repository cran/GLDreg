...
function (overall.fit.obj,alpha = 0.05, label = NULL, ColourVersion = TRUE, 
    diagnostics = TRUE, range = c(0.01, 0.99),exp=FALSE) 
{
    fit.obj <- overall.fit.obj[[1]]
    simu.obj <- overall.fit.obj[[3]]
    censoring<- fit.obj$censoring
    parEstimate <- fit.obj[[3]][match(dimnames(simu.obj)[[2]], 
        names(fit.obj[[3]]))]
        if(exp==TRUE){
        parEstimate<-exp(parEstimate)
        simu.obj<-exp(simu.obj)}
    ...
    if (diagnostics) {
        ...
        resid <- fit.obj$Residual[which(censoring==1)]
        param <- fit.obj$param
        ...
        ddst.pval <- ddst.uniform.test(pgl(resid, gld.values, 
            param = param), compute.p = T)$p.value
